module MatureFactory
  module Features
    module Assemble
      module Helpers
        def __mf_assembler_new_instance__(group, step, *attrs)
          mod = included_modules.detect {|m| m.respond_to?(:component_name) && m.component_name == group.to_s }
          if %i(flat wrap nest).include?(group)
            public_send(:"build_#{step}")
          elsif !mod.nil?
